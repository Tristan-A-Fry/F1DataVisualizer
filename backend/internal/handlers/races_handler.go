...
func RacesDataHandler(w http.ResponseWriter, r *http.Request) {
    // Get the year parameter from the URL path
    year := chi.URLParam(r, "year")

    // Fetch race data for the specified year
    data, err := services.GetRacesData(year)
    if err != nil {
        http.Error(w, "Failed to fetch race data", http.StatusInternalServerError)
        return
    }
